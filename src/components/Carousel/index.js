...
export default function Carousel({ children, refContainer }) {
  ...
  const onDragMove = useCallback(
    (e) => {
      ...
      refDragHandler.current.style.left = `${refDragHandler.current.offsetLeft - posX2.current
        }px`;
    },
    [posX1, posX2],
  )
  ...
  useLayoutEffect(() => {
    const refForwardDragonHandler = refDragHandler.current

    refForwardDragonHandler.onmousedown = onDragStart;
    refForwardDragonHandler.addEventListener("touchstart", onDragStart);
    refForwardDragonHandler.addEventListener("touchend", onDragEnd);
    refForwardDragonHandler.addEventListener("touchmove", onDragMove);
    refForwardDragonHandler.addEventListener("click", onClick);
    refForwardDragonHandler.addEventListener("transitionend", fnCheckIndex);

    return () => {
      refForwardDragonHandler.removeEventListener("touchstart", onDragStart)
      refForwardDragonHandler.removeEventListener("touchend", onDragEnd)
      refForwardDragonHandler.removeEventListener("touchmove", onDragMove)
      refForwardDragonHandler.removeEventListener("click", onClick)
      refForwardDragonHandler.removeEventListener("transitionend", fnCheckIndex)
    };
